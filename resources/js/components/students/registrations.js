...
function Registrations() {
    ...
    const data = state?.data?.registrations
    return (
        <Table scroll={{x: 20}} dataSource={data} pagination={false} rowKey={'id'}>
            <Table.Column title="Program Detail" render={(_, { ongoing_program, status }) => (
                <Space direction={'vertical'} size={1}>
                    ...
                    <p><b>End Date:</b> {ongoing_program.end_date ?? '-'} | <b>Status:</b> {status}</p>
                </Space>
            )}/>
